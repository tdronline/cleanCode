...
if (isset($_POST['install'])) {
    ...
    file_put_contents("../conf.php", "<?php define(\"PROJECT\", \"$projectPath\");  define(\"THEME\", \"$projectPath\"); ?>");

    // Clear Blacklist files
    file_put_contents("$projectPath/codepool/dev/tests/static/testsuite/Magento/Test/Less/_files/blacklist/old.txt", "");
    @file_put_contents("$projectPath/codepool/dev/tests/static/testsuite/Magento/Test/Less/_files/blacklist/ee.txt", "");
...
<div class="container">
    ...
    <?php } else { ?>
        ...
        <p>2. Run the Registry key created in php folder to set env-vars.</p>
        <p>3. Run below command to install Code Sniffer</p>
        <pre>pear install PHP_CodeSniffer</pre>
        <p>4. Run the bat file in your myerror folder to update the error report file.</p>
